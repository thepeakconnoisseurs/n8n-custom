...
| Code node Python error `"Import of external package 'X' is disallowed"` padahal X bukan kode user (insiden produksi 2026-09-04: `pytz` lalu `numexpr` dari dalam pandas) | Sandbox runner mengubah **setiap** import yang diblok allowlist jadi *Security violation* — termasuk probe opsional `try: import X except ImportError` di dalam pandas (pytz, numexpr, bottleneck, pyarrow…). Menaikkan allowlist satu-satu = whack-a-mole. | `N8N_RUNNERS_ALLOW_TRANSITIVE_IMPORTS=true` (env-override python di `n8n-task-runners.json`) — import oleh paket ter-allowlist tak dicek; import user masih dijaga. Dipilih karena semua paket di venv dikurasi Dockerfile sendiri (syarat kepercayaan versi docs). Paket yang tidak ter-install kembali gagal senyap → pandas fallback normal. |
